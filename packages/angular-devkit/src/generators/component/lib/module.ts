...
export function findModuleFromOptions(tree: Tree, options: NormalizedSchema, projectRoot: string): string {
  if (!options.module) {
    ...
    for (const c of candidatesDirs) {
      const candidateFiles = ['', moduleBaseName, `${moduleBaseName}.ts`, `${moduleBaseName}${moduleExt}`].map((x) => joinPathFragments(c, x));
...
function findModule(tree: Tree, generateDir: string, projectRoot: string): string {
  let dir = generateDir;
  const projectRootParent = dirname(projectRoot);
