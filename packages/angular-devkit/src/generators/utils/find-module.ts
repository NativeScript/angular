...
import { insertNgModuleProperty, ngModuleDecoratorProperty } from './insert-ngmodule-import';
...
export function findModule(tree: Tree, path: string, module?: string) {
  ...
  while (pathToSearch !== '.' && pathToSearch !== '/') {
    if (module) {
      ...
      const potentialOptions = tree.children(pathToSearch).filter((f) => f.endsWith('.module.ts'));
...
export function addToNgModule(
  ...
  let relativePath = `${joinPathFragments(path.replace(dirname(modulePath), ''), !isFlat ? name : '', `${fileName}`)}`;
