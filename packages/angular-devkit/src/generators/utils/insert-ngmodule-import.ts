...
import type { __String, CallExpression, ClassDeclaration, ImportDeclaration, ObjectLiteralExpression, PropertyAssignment, SourceFile } from 'typescript';
...
export function insertNgModuleProperty(tree: Tree, modulePath: string, name: string, property: ngModuleDecoratorProperty) {
  if (!tsModule) {
  ...
  const ngModuleDecorator = getDecorators(ngModuleClassDeclaration).find((decorator) => tsModule.isCallExpression(decorator.expression) && tsModule.isIdentifier(decorator.expression.expression) && decorator.expression.expression.escapedText === ngModuleName);
  ...
  if (ngModuleCall.arguments.length < 1) {
    ...
    const ngModuleOptions = ngModuleCall.arguments[0] as ObjectLiteralExpression;
...
export function insertNgModuleImport(tree: Tree, modulePath: string, importName: string) {
...
function findImport(sourceFile: SourceFile, importPath: string) {
  ...
  return importStatements.find((statement) => statement.moduleSpecifier.getText(sourceFile).replace(/['"`]/g, '').trim() === importPath);
}
...
function getNamedImport(coreImport: ImportDeclaration, importName: string) {
  ...
  return coreImport.importClause.namedBindings.elements.find((namedImport) => (namedImport.propertyName ? tsModule.isIdentifier(namedImport.propertyName) && namedImport.propertyName.escapedText === importName : tsModule.isIdentifier(namedImport.name) && namedImport.name.escapedText === importName));
}

function findDecoratedClass(sourceFile: SourceFile, ngModuleName: __String): ClassDeclaration | undefined {
  if (!tsModule) {
    tsModule = ensureTypescript();
  }
  ...
  return classDeclarations.find((declaration) => {
    ...
    if (decorators) {
      return decorators.some((decorator) => tsModule.isCallExpression(decorator.expression) && tsModule.isIdentifier(decorator.expression.expression) && decorator.expression.expression.escapedText === ngModuleName);
    }
    return undefined;
  });
}

function findPropertyAssignment(ngModuleOptions: ObjectLiteralExpression, propertyName: ngModuleDecoratorProperty) {
  if (!tsModule) {
    tsModule = ensureTypescript();
  }

  return ngModuleOptions.properties.find((property) => tsModule.isPropertyAssignment(property) && tsModule.isIdentifier(property.name) && property.name.escapedText === propertyName) as PropertyAssignment;
}
