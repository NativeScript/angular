...
describe('pipe generator', () => {
  ...
  it('should generate correctly', async () => {
    ...
    expect(tree.read('test/src/app/test.pipe.spec.ts', 'utf-8')).toMatchSnapshot();
  });
  ...
  it('should not import the pipe into an existing module', async () => {
    ...
    // ASSERT
    expect(tree.read('test/src/app/test.module.ts', 'utf-8')).not.toContain('TestPipe');
  });
  ...
  it('should not generate test file when skipTests=true', async () => {
    ...
    // ASSERT
    expect(tree.exists('test/src/app/my-pipes/test/test.pipe.spec.ts')).toBeFalsy();
  });
  ...
  describe('--no-standalone', () => {
    ...
    it('should import the pipe correctly when files are flat', async () => {
      ...
      // ASSERT
      expect(tree.read('test/src/app/test/test.pipe.ts', 'utf-8')).toMatchSnapshot();
      expect(tree.read('test/src/app/test/test.pipe.spec.ts', 'utf-8')).toMatchSnapshot();
      expect(tree.read('test/src/app/test.module.ts', 'utf-8')).toMatchSnapshot();
    });
    ...
    it('should import the pipe correctly when files are flat but deeply nested', async () => {
      ...
      // ASSERT
      expect(tree.read('test/src/app/my-pipes/test/test.pipe.ts', 'utf-8')).toMatchSnapshot();
      expect(tree.read('test/src/app/my-pipes/test/test.pipe.spec.ts', 'utf-8')).toMatchSnapshot();
      expect(tree.read('test/src/app/test.module.ts', 'utf-8')).toMatchSnapshot();
    });
...
async function generatePipeWithDefaultOptions(tree: Tree, overrides: Partial<Schema> = {}) {
