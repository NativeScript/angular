...
export async function normalizeOptions(tree: Tree, options: Schema): Promise<NormalizedSchema> {
  ...
  const { prefix } = readProjectConfiguration(tree, projectName) as AngularProjectConfiguration;

  const selector = options.selector ?? buildSelector(name, options.prefix, prefix, 'propertyName');
