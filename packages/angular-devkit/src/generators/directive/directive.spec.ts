import { addProjectConfiguration, readProjectConfiguration, updateProjectConfiguration, type Tree } from '@nx/devkit';
...
describe('directive generator', () => {
  ...
  it('should generate correctly', async () => {
    ...
    // ASSERT
    expect(tree.read('test/src/app/test.directive.ts', 'utf-8')).toMatchSnapshot();
    expect(tree.read('test/src/app/test.directive.spec.ts', 'utf-8')).toMatchSnapshot();
  });
  ...
  it('should not import the directive into an existing module', async () => {
    ...
    // ASSERT
    expect(tree.read('test/src/app/test.module.ts', 'utf-8')).not.toContain('TestDirective');
  });
  ...
  it('should not generate test file when skipTests=true', async () => {
    ...
    // ASSERT
    expect(tree.exists('test/src/app/my-directives/test/test.directive.spec.ts')).toBeFalsy();
  });
  ...
  describe('--no-standalone', () => {
    ...
    it('should import the directive correctly', async () => {
      ...
      // ASSERT
      expect(tree.read('test/src/app/test/test.directive.ts', 'utf-8')).toMatchSnapshot();
      expect(tree.read('test/src/app/test/test.directive.spec.ts', 'utf-8')).toMatchSnapshot();
      expect(tree.read('test/src/app/test.module.ts', 'utf-8')).toMatchSnapshot();
    });
    ...
    it('should import the directive correctly when directory is nested deeper', async () => {
      ...
      // ASSERT
      expect(tree.read('test/src/app/my-directives/test/test.directive.ts', 'utf-8')).toMatchSnapshot();
      expect(tree.read('test/src/app/my-directives/test/test.directive.spec.ts', 'utf-8')).toMatchSnapshot();
      expect(tree.read('test/src/app/test.module.ts', 'utf-8')).toMatchSnapshot();
    });
  ...
  describe('prefix & selector', () => {
    it('should use the prefix', async () => {
      ...
      const content = tree.read('test/src/app/example/example.directive.ts', 'utf-8');
...
async function generateDirectiveWithDefaultOptions(tree: Tree, overrides: Partial<Schema> = {}) {
  await directiveGenerator(tree, {
    name: 'test',
